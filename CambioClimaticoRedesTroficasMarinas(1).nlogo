...
breed [ fish a-fish ]  ; sheep is its own plural, so we use "a-sheep" as the singular
breed [ sharkes shark ]
...
to setup
  ...
  create-fish initial-number-fish  ; create the sheep, then initialize their variables
  [
    set shape "fish"
    set color orange
  ...
  create-sharkes initial-number-sharkes  ; create the wolves, then initialize their variables
  [
    set shape "cow"
    set color black
...
to go
  ...
  if not any? sharkes and count fish > max-fish [ user-message "The fish have inherited the earth" stop ]
  ask fish [
    move
  ...
  ask sharkes [
    ...
    reproduce-sharkes ; wolves reproduce at a random rate governed by a slider
  ]

  ask patches [ grow-fitoplancton ] 
...
to eat-fitoplancton  ; sheep procedure
  ...
  if pcolor = green [
    set pcolor sky
    set energy energy + fish-gain-from-food  ; sheep gain energy by eating
  ]
end
...
to reproduce-sharkes  ; wolf procedure
...
to display-labels
  ...
  if show-energy? [
    ask sharkes [ set label round energy ]
    ask fish [ set label round energy ]
  ]
end
...
"" ""
PENS
"fish" 1 0 -612749 true "" "plot count fish"
"sharkes" 1 0 -16449023 true "" "plot count sharkes"
"fitoplancton / 4" 1 0 -10899396 true "" "plot count fitoplancton / 4"
